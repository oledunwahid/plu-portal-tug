    ...
    db.run(`
      INSERT INTO "RequestBatchItem"
        (id, batchId, code, codeIsAutoGenerated, name, category, department, price, folder,
         serviceCharge, tax1, tax2, noDiscount, hideReceipt, printers, outlets, salesDef, barcode, sortOrder)
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    `, [
      ...
      item.printers, item.outlets, item.salesDef ?? 'SALES', item.barcode ?? null, i,
    ]);
    created.push({
      id: itemId, batchId,
      ...
      salesDef: item.salesDef ?? 'SALES', barcode: item.barcode ?? null, sortOrder: i,
    });
  }
  return created;
}
...
export interface MasterItemFilters {
  search?: string; outletGroup?: string; department?: string; active?: boolean | null;
