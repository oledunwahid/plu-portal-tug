...
interface BatchItem {
  id: string; name: string; category: string; price: number | null; code: string | null; codeIsAutoGenerated: boolean; remarks: string | null;
}
interface BatchRow {
  id: string;
  title: string;
                          ...
                          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                            ...
                            <tbody>
                              {batch.items.map((item) => (
                                <tr key={item.id}>
                                  <td style={{ padding: '0.35rem 0.5rem', fontSize: '0.8rem', fontWeight: 500 }}>
                                    {item.name}
                                    {item.remarks && item.remarks.trim() && (
                                      <div style={{ fontFamily: 'var(--font-body)', fontStyle: 'italic', color: 'var(--text-secondary)', fontSize: '0.75rem', marginTop: '2px', fontWeight: 400 }}>
                                        {item.remarks}
                                      </div>
                                    )}
                                  </td>
                                  ...
                                  <td style={{ padding: '0.35rem 0.5rem' }}>
                                    {batch.requestType === 'NEW_ITEM' ? (
                                      <>
                                        <div style={{ fontFamily: 'monospace', fontSize: '0.72rem', fontWeight: 600, letterSpacing: '0.03em', color: item.codeIsAutoGenerated !== false ? '#C9A84C' : 'var(--text-primary)', whiteSpace: 'nowrap', lineHeight: 1.2 }}>
                                          {item.code ?? '—'}
                                        </div>
                                        {item.code && (
                                          <div style={{ marginTop: '2px' }}>
                                            <span style={{ display: 'inline-block', fontSize: '0.56rem', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', padding: '1px 4px', borderRadius: '3px', background: item.codeIsAutoGenerated !== false ? 'rgba(201,168,76,0.1)' : 'rgba(122,46,31,0.1)', color: item.codeIsAutoGenerated !== false ? '#8B6914' : '#7A2E1F', border: `1px solid ${item.codeIsAutoGenerated !== false ? 'rgba(201,168,76,0.25)' : 'rgba(122,46,31,0.2)'}` }}>
                                              {item.codeIsAutoGenerated !== false ? 'AUTO' : 'MANUAL'}
                                            </span>
                                          </div>
                                        )}
                                      </>
                                    ) : (
                                      <span style={{ fontFamily: 'monospace', fontSize: '0.72rem', color: 'var(--text-secondary)' }}>{item.code ?? '—'}</span>
                                    )}
                                  </td>
                                </tr>
                              ))}
