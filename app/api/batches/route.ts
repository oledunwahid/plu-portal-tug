...
import { shouldRouteToCostControl, suggestBarcode, isWineEventCategory, STATUS_PENDING_COST_CONTROL, BARCODE_SOURCE_CASHIER, type SuggestedBarcodeResult } from '@/lib/costControl';
  ...
  try {
    ...
    if (costControlItems.length > 0) {
      ...
      for (const it of costControlItems) {
        // Barcode derivation, in priority order:
        //   1. Wine Event never generates/consumes a barcode (the filter above already excludes it;
        //      kept defensive).
        //   2. Cashier supplied a barcode → trust it 100%; no SAP fuzzy match, NCK derivation, or
        //      sequential fallback.
        //   3. Otherwise derive one from the SAP NCK sequence / sequential fallback.
        const cashierBarcode = it.barcode ?? null;
        const suggestion: SuggestedBarcodeResult = isWineEventCategory(it.category)
          ? { value: null, source: null }
          : cashierBarcode && cashierBarcode.trim() !== ''
            ? { value: cashierBarcode, source: BARCODE_SOURCE_CASHIER }
