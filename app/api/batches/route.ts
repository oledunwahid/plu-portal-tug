...
export async function POST(req: NextRequest) {
  try {
    ...
    // All items — including WINE NEW_ITEM from Cork outlets — go into the batch as PENDING and enter
    // the normal admin queue. (The former per-item cost-control approval stage was removed because the
    // system cannot reliably distinguish bottle items from non-bottle items.)
    ...
    return NextResponse.json(
      { ...batch, warnings: warnings.length > 0 ? warnings : undefined },
      { status: 201 },
    );
  } catch (error) {
