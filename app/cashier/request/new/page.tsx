...
export default function NewRequestPage() {
  ...
  // Build derived data from fetched config
  ...
  const showName = form.requestType === 'NEW_ITEM' || form.requestType === 'UPDATE_NAME' || form.requestType === 'UPDATE_FULL';
  const showCategoryDept = form.requestType === 'NEW_ITEM' || form.requestType === 'UPDATE_FULL';
  const showPrice = form.requestType === 'NEW_ITEM' || form.requestType === 'UPDATE_PRICE' || form.requestType === 'UPDATE_FULL';
  const showFolder = form.requestType === 'NEW_ITEM' || form.requestType === 'UPDATE_FULL';
  const showPrintersCard = form.requestType === 'NEW_ITEM' || form.requestType === 'UPDATE_PRINTER' || form.requestType === 'UPDATE_FULL';
  const showOutletsCard = true;
  const showPOSCard = form.requestType === 'NEW_ITEM' || form.requestType === 'UPDATE_FULL';
  const showSalesDef = form.requestType === 'NEW_ITEM' || form.requestType === 'UPDATE_FULL';
  ...
  function validate(): boolean {
    ...
    if (form.requestType === 'NEW_ITEM') {
      ...
      if (form.category && !form.code.trim()) errs.code = 'A PLU code is required. Enter a code manually or ensure the category is mapped.';
      ...
      if (!form.code.trim()) errs.code = 'PLU code is required';
      if (form.printers.length === 0) errs.printers = 'Select at least one printer';
    } else if (form.requestType === 'UPDATE_FULL') {
      if (!form.code.trim()) errs.code = 'PLU code is required';
    }
    setErrors(errs);
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            {showSalesDef && (
              <FieldGroup label="SALES DEF">
                <select
                  value={form.salesDef}
                  onChange={(e) => set('salesDef', e.target.value as 'SALES' | 'MODIFIER')}
