...
export default function BatchEditPage() {
    ...
    fetch(`/api/batches/${batchId}`)
      ...
      .then((data) => {
        ...
        if (data.requestType === 'UPDATE_PRICE' || data.requestType === 'UPDATE_NAME') {
          ...
          if (codes.length > 0) {
            fetch('/api/admin/kb/items/lookup', {
              method: 'POST',
              ...
              .then((res) => {
                if (!res?.items) return;
                const byCode = new Map<string, { name: string; price: number | null }>();
                for (const mi of res.items as any[]) {
                  byCode.set(mi.code, { name: mi.name ?? '', price: mi.price ?? null });
                }
                setItems((prev) => prev.map((p) => {
                  const ref = byCode.get(p.code);
                  return ref
                    ? { ...p, currentName: ref.name, currentPrice: ref.price != null ? String(ref.price) : '' }
                    : p;
                }));
              })
              .catch(() => { });
  ...
  const addRow = useCallback(() => {
    setItems((prev) => {
      ...
      return [...prev, { ...last, _id: uid(), code: '', name: '', remarks: '', currentName: '', currentPrice: '', errors: {} }];
    });
  }, []);
            ...
            <table style={{ tableLayout: 'auto', minWidth: isRemovePLU ? '600px' : '900px' }}>
              <thead>
                <tr>
                  ...
                  {!isRemovePLU && isUpdate && <th style={{ minWidth: '200px' }}>PLU Code</th>}
