...
interface ItemRow {
  ...
  barcodeQuery: string; // UPDATE_PRICE barcode-scan search box (independent of code/name)
  barcodeSapNotice: string | null; // set when a scan matched SAP but not the Quinos master
...
function makeDefaultRow(): ItemRow {
  return {
    ...
    salesDef: 'SALES', printers: [], outlets: [], remarks: '', currentName: '', currentPrice: '',
    barcodeQuery: '', barcodeSapNotice: null, errors: {},
  };
}
...
export default function BatchNewPage() {
  ...
  const addRow = useCallback(() => {
    setItems((prev) => {
      ...
      const newRow: ItemRow = {
        ...
        currentName: '', currentPrice: '', barcodeQuery: '', barcodeSapNotice: null, errors: {},
      };
      return [...prev, newRow];
    });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  function duplicateRow(idx: number) {
    setItems((prev) => {
      const src = prev[idx];
      const copy: ItemRow = { ...src, _id: uid(), name: '', remarks: '', barcodeQuery: '', barcodeSapNotice: null, errors: {} };
  ...
  // UPDATE_PRICE barcode-scan box (per row). Manual typing keeps code/name untouched.
  function handleBarcodeQueryChange(idx: number, v: string) {
    setItems((prev) => prev.map((r, i) => (i === idx ? { ...r, barcodeQuery: v, barcodeSapNotice: null } : r)));
  }

  // Barcode scan selection: a verified master hit fills code + current name/price like
  // the other search fields; a SAP-only hit has no master code, so we just echo the name
  // and flag it as not-yet-verified in Quinos.
  function handleBarcodeItemSelect(idx: number, item: PLUSearchResult) {
    setItems((prev) => prev.map((r, i) => {
      if (i !== idx) return r;
      if (item.sapOnly) {
        return { ...r, currentName: item.name, currentPrice: '', barcodeSapNotice: item.name };
      }
      return {
        ...r, code: item.code, codeIsAutoGenerated: false,
        currentName: item.name, currentPrice: item.price != null ? String(item.price) : '',
        barcodeSapNotice: null, errors: { ...r.errors, code: undefined },
      };
    }));
  }
            ...
            <table style={{ tableLayout: 'auto', minWidth: isRemovePLU ? '600px' : '900px' }}>
              <thead>
                <tr>
                  ...
                  {!isRemovePLU && requestType === 'UPDATE_PRICE' && <th style={{ minWidth: '180px' }}>Barcode Scan</th>}
                ...
                {items.map((row, idx) => {
                  ...
                  return (
                    <tr key={row._id} style={{ borderLeft: hasError ? '2px solid rgba(122,46,31,0.6)' : undefined, verticalAlign: 'top' }}>
                      ...
                      {/* Barcode scan - UPDATE_PRICE only. Independent of the PLU code / name search;
                          auto-submits on Enter for physical scanners. */}
                      {!isRemovePLU && requestType === 'UPDATE_PRICE' && (
                        <td>
                          <PLUCodeSearch
                            mode="barcode"
                            value={row.barcodeQuery}
                            onChange={(v) => handleBarcodeQueryChange(idx, v)}
                            onItemSelect={(item) => handleBarcodeItemSelect(idx, item)}
                            placeholder="Scan barcode…"
                          />
                          {row.barcodeSapNotice && (
                            <p style={{ fontSize: '0.68rem', color: '#8B6914', margin: '2px 0 0' }}>
                              SAP: belum terverifikasi di master
                            </p>
                          )}
                        </td>
                      )}
